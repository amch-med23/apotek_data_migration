...
cat clinical_drug_recommendations.json | jq -c '.[]' | while read clinical_rec; do
    ...
    # Extract dosing_details and ensure it's a valid JSON object
    dosing_details=$(echo $clinical_rec | jq -c '.dosing_details // {}' | sed "s/'/''/g")
    
    # Extract usage_notes and escape single quotes
